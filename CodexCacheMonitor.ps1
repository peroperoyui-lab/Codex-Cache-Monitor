...
try {
    ...
    $subtitleLabel.Text = '读取本地 rollout JSONL；Cached 是 Input 的子集。关闭本窗口即退出。'
    ...
    $chooseButton.Text = '选择 JSONL…'
    $chooseButton.Size = New-Object System.Drawing.Size(105, 28)
    $chooseButton.Location = New-Object System.Drawing.Point(190, 77)
    $form.Controls.Add($chooseButton)

    $copyButton = New-Object System.Windows.Forms.Button
    $copyButton.Text = '复制摘要'
    $copyButton.Size = New-Object System.Drawing.Size(90, 28)
    $copyButton.Location = New-Object System.Drawing.Point(305, 77)
    $form.Controls.Add($copyButton)

    $exitButton = New-Object System.Windows.Forms.Button
    $exitButton.Text = '退出'
    $exitButton.Size = New-Object System.Drawing.Size(75, 28)
    $exitButton.Location = New-Object System.Drawing.Point(405, 77)
    $form.Controls.Add($exitButton)

    $fileCaption = New-Object System.Windows.Forms.Label
    $fileCaption.Text = '当前会话：'
    ...
    $fileLabel.Location = New-Object System.Drawing.Point(90, 116)
    $fileLabel.Size = New-Object System.Drawing.Size(655, 22)
    $form.Controls.Add($fileLabel)

    $toolTip = New-Object System.Windows.Forms.ToolTip

    $updatedLabel = New-Object System.Windows.Forms.Label
    $updatedLabel.Text = '状态：等待数据'
    ...
    $grid.Controls.Add((New-GridLabel '指标' $true), 0, 0)
    $grid.Controls.Add((New-GridLabel '本会话累计' $true), 1, 0)
    $grid.Controls.Add((New-GridLabel '最近一次请求' $true), 2, 0)

    $metricNames = @(
        @{ Key = 'Input';     Label = 'Input tokens（含缓存）' },
        ...
        @{ Key = 'Reasoning'; Label = 'Reasoning output（Output 子集）' },
    ...
    $noteLabel.Text = '注：当前 Codex 某些 GPT-5.6 构建不会把 cache-write token 写进 rollout，因此本工具只可靠显示 cache read。'
    ...
    function Set-MetricLabels {
        ...
        if (-not [double]::IsNaN($LastMetrics.HitRate)) {
            $cacheText.Text = '最近请求缓存命中率：' + (Format-Percent $LastMetrics.HitRate)
        ...
        if ($ContextWindow -gt 0 -and $LastMetrics.Input -gt 0) {
            ...
            $contextText.Text = ('最近请求上下文压力：{0} / {1}  ({2})' -f (Format-TokenCount $LastMetrics.Input), (Format-TokenCount $ContextWindow), (Format-Percent $contextPercent))
    ...
    function Update-Monitor {
        ...
        try {
            ...
            if ([string]::IsNullOrWhiteSpace($script:currentPath)) {
                $fileLabel.Text = '未找到 rollout JSONL'
                $updatedLabel.Text = '状态：请先运行一次 Codex，或手动选择 JSONL。'
                return
            }
            ...
            if (-not (Test-Path -LiteralPath $script:currentPath -PathType Leaf)) {
                ...
                $updatedLabel.Text = '状态：当前 JSONL 不存在。'
                return
            }
            ...
            if ($signature -eq $script:lastSignature) {
                if ($null -ne $script:lastBundle) {
                    ...
                    $updatedLabel.Text = ('状态：已连接；文件 {0:N0} KB；最后写入 {1} 秒前' -f ($fileInfo.Length / 1KB), $age)
                }
                return
            }
            ...
            if ($null -eq $line) {
                if ($null -ne $script:lastBundle) {
                    $updatedLabel.Text = '状态：文件有更新，但尾部暂未找到新的 token_count；保留上次读数。'
                }
                else {
                    $updatedLabel.Text = ('状态：在文件尾部 {0} MB 内尚未找到 token_count。' -f $MaxTailMB)
                }
            ...
            if ($null -eq $bundle) {
                $updatedLabel.Text = '状态：找到 token_count，但当前版本字段结构无法解析；保留上次读数。'
                $script:lastSignature = $signature
                return
            }
            ...
            $script:lastSignature = $signature
            $updatedLabel.Text = ('状态：已更新；文件 {0:N0} KB；写入时间 {1}' -f ($fileInfo.Length / 1KB), $fileInfo.LastWriteTime.ToString('HH:mm:ss'))
        }
        catch {
            $updatedLabel.Text = '状态：' + $_.Exception.Message
        }
    }

    $chooseButton.Add_Click({
        $dialog = New-Object System.Windows.Forms.OpenFileDialog
        $dialog.Title = '选择 Codex rollout JSONL'
    ...
    $copyButton.Add_Click({
        if ($null -eq $script:lastMetrics) {
            [System.Windows.Forms.MessageBox]::Show('还没有可复制的 token 数据。', 'Codex Cache Monitor') | Out-Null
            return
        }
...
"@
        ...
        $updatedLabel.Text = '状态：摘要已复制到剪贴板。'
    })
